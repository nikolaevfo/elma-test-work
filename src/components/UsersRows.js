export default class UsersRows {
  ...
  renderRowsItems(usersData, tasksData, firstDay, quantity, onDropHandler) {
    usersData.forEach((item) => {
      this._addItem(
        this._generateRow(item, tasksData, firstDay, quantity, onDropHandler)
      );
    });
  }
  ...
  _addTasks(thisUserTasks, thisCardDate, newCard, todayData) {
    if (thisUserTasks.length > 0) {
      thisUserTasks.forEach((element) => {
        ...
        newUserTask.querySelector(
          '.board__tasks-user-list-item-header'
        ).textContent = element.subject;
        newUserTask.querySelector(
          '.board__task-submenu-item-description'
        ).textContent = element.description;
        newUserTask.querySelector(
          '.board__task-submenu-item-creationDate'
        ).textContent = element.creationDate;
        newUserTask.querySelector(
          '.board__task-submenu-item-planStartDate'
        ).textContent = element.planStartDate;
        newUserTask.querySelector(
          '.board__task-submenu-item-planEndDate'
        ).textContent = element.planEndDate;

        // проверка, просрочено ли задание
        const thisCardDateNewFormat = thisCardDate
          .split('.')
          .reverse()
          .join('-');
        ...
        newCard
          .querySelector('.board__tasks-card-item-list')
          .append(newUserTask);
      });
    }
  }
